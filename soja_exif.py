...
"""
什么是EXIF信息呢？
    百度百科：Exif是一种图象文件格式，它的数据存储与JPEG格式是完全相同的。
实际上Exif格式就是在JPEG格式头部插入了数码照片的信息，包括拍摄时的光
圈、快门、白平衡、ISO、焦距、日期时间等各种和拍摄条件以及相机品牌、型
号、色彩编码、拍摄时录制的声音以及全球定位系统（GPS）、缩略图等。所有
的JPEG文件以字符串“0xFFD8”开头，并以字符串“0xFFD9”结束。文件头中有
一系列“0xFF??”格式的字符串，称为“标识”，用来标记JPEG文件的信息段。
“0xFFD8”表示图像信息开始，“0xFFD9”表示图像信息结束，这两个标识后面
没有信息，而其它标识紧跟一些信息字符。0xFFE0 — 0xFFEF之间的标识符称
为“应用标记”，没有被常规JPEG文件利用，Exif正是利用这些信息串记录拍摄
信息的。
    逛摄影论坛时经常会看到，照片的底部包含很多其他信息，如：曝光度，光圈，
焦距，快门，机身等等，这些信息就是EXIF信息，摄影爱好者可以参考这些信
息提高自己的摄影技术。本文主要涉及的是如何把信息隐藏到图片中，比如一
个电影地址。
    首先实现一个最简单的方式，把信息直接添加到图片的头部或者尾部，直
接添加到头部由于破坏了图片的数据，所以头部会出现一块黑色的区域比较明
显，所以别人一下子就看出来了，效果最差。添加到尾部只是简单的增加了图
片的大小，图片的数据区域并没有改变，所以如果信息量不是很大，基本是看
不出来的，缺点是传到其他网站时容易被裁剪掉。下面的代码实现了把种子隐
藏到图片尾部的1024字节区域。
